...
function getTotalCount(){
    ...
    .then(response=>{// promise가 response =>{}안에 들어간다고 보면 됨
        ...
        if(response.ok){
            ...
            //  text(string) 형태로 변환(parsing)(Java형태로 변환하는거임)
    ...
    // 첫 번째 then에서 더져진 Error를 잡아서 처리하는 구문
...
function getCompleteCount(){
    ...
    - 매개변수 response :
            응답 결과, HTTP 상태 코드, 요청주소 등이 담겨있는 객체
...
.then(result => {
    // JSON(List 형태)을 JS 객체 배열로 변경
    const todoList = JSON.parse(result);
    console.log(todoList);

    /* #tbody내용을 모두 지운 후 조회된 내용의 요소 추가 */
...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    document.querySelectorAll("#tbody a").forEach((a)=>{
                // 매개변수 a : 반복마다 요소가 하나씩 꺼내져서 저장되는 변수
 ...
 * @param url : /todo/detail/10 <- 요런 행태
 */
function selectTodo(url){
    ...
    .then(response=>{
        if (response.ok) { // 요청 / 응답 성공 시
            ...
            // response.json()
            //  -> ressponse.text() + JSON.parse() 합친 메서드
...
updateView.addEventListener("click",()=>{
  ...
  // - datasset속성 : 요소에 js에서 사용할 값(data)를 추가하는 속성
  // 요소.dataset.속성명 = '값' -> 대입
