...
ex) 전화
 ...
 - ws:// 프로토콜 요청 (http가 아님)
		...
		- 어떤 HTTP요청 주소가 Websocket 요청인지 지정
*/
...
const chattingEnter = (e) => {
  ...
  .then(chattingNo => {
    ...
    console.log(chattingNo);
		...
		selectRoomList(); // 비동기로 채팅방 목록을 조회

		// 200ms 후에 실행
		setTimeout(()=>{

			// 입장하려던 채팅방이 이미 채팅방 목록에 존재하는 경우

			// 1) 채팅방 목록 li 태그들 얻어오기
			const itemList = document.querySelectorAll(".chatting-item");

			// 2) li태그의 "chat-no" 값과 입장하려는 방 번호가 같은 경우
