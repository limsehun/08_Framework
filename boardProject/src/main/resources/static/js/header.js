...
- 단방향 통신 (ex : 무전기)
 ...
 1) 클라이언트가 서버에 연결 요청
  -> 클라이언트가 서버로부터 데이터를 받기 위한 대기상태 돌입(EventSource 객체 이용)
...
/** SSE연결하는 함수
 * -> 연결을 요청한 클라이언트가 서버로부터 
 * 데이터가 전달될 때 까지 대기 상태가 됨(비동기)
 * 
 */
const connectSse = () => {
    ...
    eventSource.addEventListener("message", e=>{
        ...
        console.log(obj); // 알림을 받는사람 번호, 읽지 않은 알림 개수
...
2) 연결된 클라이언트 대기 명단(emitters)에서 클라이언트 id가 일하는 회원을 찾아
   메세지를 전달하는 send() 메서드 수행

3) 서버로부터 메세지를 전달 받은 클라이언트의 EventSource.addEventListener()가 수행됨
