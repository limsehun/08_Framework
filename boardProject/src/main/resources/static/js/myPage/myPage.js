...
memberNickname?.addEventListener("input", ()=>{
    ...
    // 비동기로 입력된 닉네임이 데이터 베이스에 존재하는지
    // 확인하는 Ajex코드(fetch()API)작성
...
updateInfo?.addEventListener("submit", e=>{
    ...
    // for ~ in 구문 : JS 객체의 key값을 하나씩 접근하는 반복문5
...
changePw?.addEventListener("submit", e => {
  ...
  let str; // undefined 상태(자료형 X)

  if(currentPw.value.trim().length==0)
    str= "현재 비밀 번호를 입력해 주세요";
...
// 프로필 이미지 업로드 상태에 따라 어떤 상태인지 구분하는값
...
if (imageInput != null) { // 프로필 변경 화면인 경우
    ...
    */
    const updatePreview = (file) => {
        ...
        // DataURL : 파일 전체 데이터가 브라우저가 해석할 수 있는 긴 주소 형태 문자열로 변환
    ...
    imageInput.addEventListener("change",e => {
        ...
        if(file===undefined){
            /* 이전에 선택한 파일을 유지하는 코드 */
            // -> 이전 선택한 파일을 저장할 변수 (lastVaildFile)선언
            ...
            // dataTransfer객체가 가지고 있는 file 필드에 lastVaildFile추가

            // input의 files(필드)변수에 lastVaildFile이 추가된 files대입
...
const profileFronm = document.querySelector("#profile");

profileFronm?.addEventListener("submit",e=>{
