	...
	public String test2(MultipartFile uploadFile, String fileName) throws IllegalStateException, IOException {
		...
		// if(uploadFile.getOriginalFilename().equals(""))
		 if(uploadFile.isEmpty()) {
			 return null; // 업로드된 파일이 없을때 null 반환
		 }
...
//			if(a == 1) throw new RuntimeException();
			...
			// 3) try-catch 작성시 메서드 내부에서 예외처리되어 메서드 종료시 예외가 던져지지 않아
			//    @Transactional이 rollback을 수행할 수 없게된다.
			
			// [추천되는 해결 방법]
			// * try-catch를 작성해서 Checked Exception을 처리 - 호출하는 메서드에 throws 구문 작성 X
			
			// * Unchecked Exception 형태의 사용자 정의 예외 강제 발생
			//  - @Transactional 어노테이션에 rollbackFor 속성 작성안해도 롤백 처리 가능
			
			
			// 예외 강제 발생
			// - Unchecked Exception은 컴파일러가 자동으로 throws 구문을 작성해줘서 예외 발생 시 호출부로 던져지게됨
