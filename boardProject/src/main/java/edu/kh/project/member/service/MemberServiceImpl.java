...
@Slf4j
@Service //  비즈니스 로직을 처리하는 역할 명시 + Bean등록
public class MemberServiceImpl implements MemberService{
	  ...
	  private BCryptPasswordEncoder encoder;
		 ...
		 * 	
		 * - 약간 과거 또는 현재 : 데이터를 암호화만 가능(SHA-512,256/ SHA방식) -> 복호화 방법 제공 X
		 * 
		 ...
		 * -> 암호화 할 때 마다 결과가 다름
		 * -> DB에 입력 받은 비밀번호를 암호화 해서 넘겨줘도 비교 불가능
		 * -> BCrypt가 함께 제공하는 평문, 암호화 데이터 비교 메서드인
		 * 	  matches()를 이용하면된다.(같은면 true, 다르면 false)
		 * 
