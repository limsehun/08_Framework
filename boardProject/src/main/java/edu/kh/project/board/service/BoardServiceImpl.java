...
public class BoardServiceImpl implements BoardService{
	...
	@Override
		...
		/* [boardNo처럼 하나의 값을 이용해 여러번 SELECT수행하는 경우] 
		 * 
		 * 1. 하나의 SERVICE 메서드에서 여러 MAPPER 메서드 호출하기
		 * 		service -> mapper -> DB (필요한 SELECT만큼 전체 반복)
		 * 
	...
	// 검색목록 조회
	@Override
	public Map<String, Object> selectBoardList(int boardCode, int cp, Map<String, Object> paramMap) {
		
		// 지정된 게시판에서 검색 조건이 일치하는 게시글이 몇 개나 존재하는지 조회
		
		paramMap.put("boardCode", boardCode); // boardCode 도 paramMap에 추가
