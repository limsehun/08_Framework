...
public class EditBoardServiceImple implements EditBoardService{
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) {
		...
		for(int i=0 ; i < images.size(); i++) {
			...
			BoardImg img = BoardImg.builder()
												...
												.imgRename(rename) // 웹 접근 경로
												.imgPath(webPath) // 이미지가 삽입된 
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList) {
		...
		// 2. 기존에 존재하던 이미지 중 DeleteOrderList에 존재하는 순서의 이미지 DELETE
