...
import org.springframework.stereotype/*이게 뒤에오는 거다 라고 명시*/.Controller;
...
//Argument  : 전달 인자(메서드 호출시 전달하는 값)

// Controoler : 요청, 응답 제어 역할 
// Bean 			: Spring이 만든 객체
...
//@RequestMapping(value="test1",method = RequestMethod.Get) == @GetMapping("test1")
	@GetMapping("test1")
  ...
  - 여러 개(복수, 다수) 파라미터 컨트롤러에서 얻는 방법 */
	...
	public String test3(		
			...
			// 파라미터 중 color만 모아서 String[]로 전달받기
			// 단, 필수는 아니다
			
			@RequestParam(value="fruit", 
			required = false/*이거 필수 아니야 라는 구문*/) List<String> fruitList,
			
			@RequestParam Map<String, Object> paramMap
			// 위의 2개와 달리()가 없다 -> ㅡMap 형식으로 파라미터를 얻어오는 경우
			// 특정 key 만 얻어오는게 아닌 모든 파라미터를 얻어옴
			// 단, key가 중복인 경우 value 덮어쓰기 이루어짐
			) {

		log.debug("colorArr : {}", Arrays.toString(colorArr));
		log.debug("fruitList : {}", fruitList);
		// 둘 다 괜찮지만 List가 편하다고 한다
		log.debug("paramMap : {}", paramMap);
		
		return "param/result3"; // param폴더 안의 result3파일에 보내겠다(forward)
	}
	...
	public String test4(
		 ...
		 * 
		 * name 속성== value 속성(같은 속성임)
		 * 
		 * * required : 필수 (input type text엔 무쓸모)
		 *  -> 기본 값이 true 
	...
	// 많은 양의 파라미터를 하나의 DTO로 한 번에 저장할 수 있어 편함
	...
	public String test5(
			...
			/*@ModelAttribute */Member member2/* == 커맨드 객체, Mrmber DTO에서 가져온다 (DTO만 잘 만들어 두면 편하다) */

			) {
			
			// Member 객체를 생성해서 전달받은 값 세팅하기
		...
		log.debug("member2 : {}", member2); //== 커맨드 객체
